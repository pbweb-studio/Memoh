...
- Шаблоны из `deploy/studio-jarvis-native/data/*` скопированы в `studio/` для бота `994b7558-a192-4a5a-991d-b6dac6ca647e`. **Managed skills:** четыре `SKILL.md` из `deploy/studio-jarvis-native/skills/*` зарегистрированы через **`POST /api/bots/{bot_id}/container/skills`** (эквивалент UI **Skills → New Skill**): сервер пишет в **`/data/skills/<name>/SKILL.md`** через workspace bridge; только bind-mount на хост без этого upsert давал пустой **`GET .../container/skills`** → **«No skills yet»**.
